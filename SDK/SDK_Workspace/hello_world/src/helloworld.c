...
{
    init_platform();
    unsigned char string_s[] = "MIA I LALE\n";
    int x = 0;
    int y = 0;
    int mia = 1;
    int lale = 1;
   ...
   // draw_square1(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR);
    set_cursor(400);
    print_string(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR, string_s, 10);

    while(1){

    	pomeraj_se(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,x,y);
    	x+=mia;
   ...
   /* while(1){
       	...
       	pomeraj_se1(XPAR_VGA_PERIPH_MEM_0_S_AXI_MEM0_BASEADDR,x,y);
       	x+=lale;
